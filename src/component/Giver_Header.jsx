...
export default function Header() {
  ...
  // 특정 페이지 목록
  const onlyExitPages = ["/groupcreate", "/groupstatistics"];

  // 현재 페이지가 위 배열 중 하나인지 체크
  const isOnlyExitPage = onlyExitPages.includes(location.pathname);

  const goToMain = () => {
    // 만약 onlyExitPage 라면 시작점으로 이동
    if (isOnlyExitPage) {
      navigate("/giver-main");
    } else {
      navigate("/main");
  ...
  return (
    <div className="header-grid">
      ...
      {/* onlyExitPage 이면 다른 버튼 숨기기 */}
      {!isOnlyExitPage && !isLookBookPage && (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="27"
          height="26"
